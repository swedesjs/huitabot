declare global {
  ...
  declare global {
    ...
    declare type NextTypes<T extends MessageContext | MessageEventContext = MessageContext> = Parameters<Parameters<HearManager<T>["hear"]>[1]>

    type commandTypes = Readonly<{
      hearConditions: HearConditions<MessageContext>
      handler: (ctx: MessageContext, next?: NextTypes[1]) => void
    }>
    type eventTypes = Readonly<{
      hearConditions: HearConditions<MessageEventContext>
      handler: (ctx: MessageEventContext, next?: NextTypes<MessageEventContext>[1]) => void
    }>
  }
}
